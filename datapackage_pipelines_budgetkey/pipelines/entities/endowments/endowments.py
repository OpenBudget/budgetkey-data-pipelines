...
def flow(*_):
    return DF.Flow(
        ...
        DF.set_type('areas_of_activity', type='array', **{'es:itemType': 'string'}, transform=lambda x: x.split(',') if x else []),
        DF.set_type('areas_of_activity_secondary', type='array', **{'es:itemType': 'string'}, transform=lambda x: x.split(',') if x else []),
